...
`apps/web/` — a static, dependency-free marketing site (plain HTML + CSS + vanilla JS,
deployed on Vercel with `outputDirectory: "."`, no build step). Pages are top-level `.html`
files; styles live in `css/`, behaviour in `js/`, images and vectors in `assets/`.
See `apps/web/README.md` for the scroll-scrub hero and animation details.

There is no bundler, framework, or test runner — keep changes to plain, standards-based
HTML/CSS/JS and verify by opening the pages.
